...
class LogUtil(object):
    ...
    def get_logger(cls, source_filepath=None, level=logging.DEBUG, format=format, datefmt=datefmt, console_mode=False):
        ...
        with lock:
            ...
            if not console_mode and (platform.system().lower() == 'darwin'):
                console_mode = True
            ...
            if log_path:
                ...
                if log_path and not os.path.exists(os.path.dirname(log_path)):  # create log dir
                    os.makedirs(os.path.dirname(log_path))
                if error_log_path and not os.path.exists(os.path.dirname(error_log_path)):  # create log dir
                    os.makedirs(os.path.dirname(error_log_path))

            # print('log_path:', log_path)
            # print('error_log_path:', error_log_path)

            cls.__log = LogUtil.__create_logger(log_path, error_log_path, level, console_mode, format, datefmt)
    ...
    def __log_path_from_path(source_file, sub_log_dir='logs'):
        ...
        if len(sys.argv) > 1:
            _basename = '%s.%s' % (_basename, LogUtil.to_filename('.'.join(sys.argv[1:])))
        log_path = os.path.join(_dir, _basename) + '.log'
        return log_path

    @staticmethod
    def __create_logger(log_path, error_log_path=None, level=logging.NOTSET, console_mode=False, format=format, datefmt=datefmt):
        ...
        log = logging.getLogger(log_path)  # for single process but MultProcTimedRotatingFileHandler support multiprocessing
        ...
        if log_path:
            # _fileHandler = TimedRotatingFileHandler(filename=log_path, when='midnight', interval=1, backupCount=0)
            _fileHandler = MultProcTimedRotatingFileHandler(filename=log_path, when='midnight', interval=1, backupCount=0)
            _fileHandler.setLevel(level)
            _fileHandler.setFormatter(formatter)
            log.addHandler(_fileHandler)

            if error_log_path:
                _file_errorHandler = MultProcTimedRotatingFileHandler(filename=error_log_path, when='midnight', interval=1, backupCount=0)
